...
- 浏览器会话保留独立可切换软字幕；PotPlayer/IINA 外部会话在选中字幕时强制 HLS，并把该轨道追加在增强/模型输出之后烧录。代码已探测并缓存 FFmpeg `subtitles`/libass 能力，缺失时返回 `MEDIA_SUBTITLE_BURN_UNAVAILABLE`。仍须分别用 ASS/VTT 在 PotPlayer 和 IINA 验证样式、中文字体、跳转时间轴和字幕不被模型锐化后，才能记录 `device-passed`。
- 普通 HLS 和模型 rawvideo 链都验收 NVENC/AMF/QSV/libx264；模型链必须对每个探测通过的候选重建解码器、模型管线和编码器，清理上一轮播放列表与分片后再尝试下一候选。代码已覆盖最后失败原因和 `libx264` 降级语义；未完成对应 GPU 真机证据前不得记录跨厂商模型编码通过。
- 网页 HLS 致命错误的代码路径会以当前绝对时间自动重建会话，最多两次，超过上限保留手动重试。验收时人为中断网络，确认新会话、HLS 清单、播放位置和字幕状态可恢复，并记录重连耗时和是否达到上限。
