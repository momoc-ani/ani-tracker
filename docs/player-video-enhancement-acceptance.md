...
- F5-A 能力探测已实现并有共享单元测试，覆盖 Audio/Video WebCodecs、WebGPU、候选 codec 和内置 WGSL SHA-256；F5-B 已加入受控 MP4/WebM Range demux 诊断、实际 decoder config、关键帧和绝对时钟基础；F5-C 已加入外部 VideoFrame shader 工厂及 1x1 OffscreenCanvas 首帧 smoke test。在可见 shader 画布接管、音频样本调度和时钟接管完成前，`supportsDirectEnhancement` 必须保持 `false`。
...
- WebGPU shader 只处理视频帧，当前 F5-C 工厂尚未接管现有画布；正式接管后字幕和 OSD 必须在 shader 输出后叠加，切换字幕、字幕缩放和 ASS/VTT 不得导致文字被锐化或重复绘制。
