...
- WebGPU/WebCodecs 直传增强：容器解复用、音频时钟、关键帧跳转、浏览器能力矩阵和 GPU 资源回收。
...
| 终版 F5 | 计划新增 WebCodecs/WebGPU 直传增强，不修改原文件和服务端直传协议 | 尚未开始；先完成能力 PoC，再进入容器、时钟、shader、字幕和浏览器矩阵验收 |
