...
> 当前实现边界（2026-08-14）：桌面 Windows/Linux/macOS 已统一使用 libmpv，接入 gpu-next/Render API、平台硬解路径、Anime4K Shader、字幕后合成、掉帧自动降级和双窗口同步；PC 播放路径不再保留 libVLC。远程 HLS 已按 NVENC/AMF/QSV/libx264 探测普通转码编码器；模型链已接入 RIFE 插帧和 Real-ESRGAN 2x 动画超分，采用 `FFmpeg RGB24 解码 -> 可选 RIFE -> 可选 Real-ESRGAN -> FFmpeg 编码`，并通过受认证会话诊断报告实际模型与降级原因。RIFE 已增加按源帧率、模型滚动 P95、显存、60 FPS 输出上限、80% 利用率和 5 ms 链路余量计算的容量门禁，当前协议因 timestep 固定为 `0.5` 而硬限制为 2x；运行时超预算会关闭模型并维持既定时间轴。macOS Intel + AMD RX 6750 XT + MoltenVK 已完成真实 sidecar 握手：Real-ESRGAN 最新 warmup `24.46 ms`；RIFE 最新 warmup `25.99 ms`。这些低分辨率 warmup 只可作为启动样本，真实分辨率滚动 P95 和完整播放矩阵未通过前不能标记为实时插帧验收通过。F5-A/F5-B/F5-C 已完成能力、受控 MP4/WebM Range demux、实际解码配置、关键帧/首个音频样本、内置 WGSL、1x1 首帧 smoke test；F5-D 已接入最多 48 帧的有界 VideoFrame 队列、连续 WebCodecs 解码、可见 WebGPU 画布、预设切换、关键帧拖动和原画回退；F5-E 已接入 WebGPU device-lost、按源帧率计算的 80% 帧预算、GPU 队列 P95、丢帧率、连续音画漂移和无帧超时门禁，清晰档超限先自动降到均衡；F5-F 已接入 AudioBufferSink 内部 AudioDecoder、AudioContext 主时钟、1.5 秒/96 buffer 有界预调度，以及暂停、拖动、倍速、音量、静音和结束态；F5-G 已接入 GPU 纹理上限、有界队列工作集估算和 WebGPU `out-of-memory` 运行门禁。增强激活后卸载 ArtPlayer 原媒体源，ASS/VTT 由独立 cue 时钟直接渲染到 ArtPlayer DOM，连续重复 Range/视频解码已从增强路径移除。WebGPU 不提供真实显存遥测，因此 F5-G 只报告估算预算，不伪报 VRAM；F5-H 已加入最多两次 Range 建连、状态和响应体续传恢复，远程 HLS 会话也已加入两次有界自动重建；F5-I 已加入受认证、设备绑定、单调序号的终端诊断上报，服务端会话可区分 `direct`、`direct-enhanced` 与 `hls`。Intel Mac + AMD RX 6750 XT + Chrome 的 1080p MP4 直传增强已通过 30:40 `device-passed`，完整浏览器/GPU 矩阵仍未完成。
...
普通 HLS 与模型 rawvideo 链都按 NVIDIA NVENC、AMD AMF、Intel QSV 选择，全部不可用时回退 libx264。模型链会对每个探测通过的候选重新创建解码器、模型管线和编码器，启动失败后清理播放列表与分片再尝试下一候选；成功后报告实际编码器，全部失败时返回最后候选的结构化原因。跨厂商真机证据仍属于后续发布门禁。
...
6. **远程增强输出**：基础 HLS 已按 NVENC/AMF/QSV/libx264 探测并报告降级；RIFE 与 Real-ESRGAN 已进入独立 rawvideo 管线，音轨和软字幕不经过模型，实际模型后端与运行时降级通过 2 秒诊断轮询展示。模型链已支持逐候选编码器启动重试；网页 HLS 致命错误会从当前绝对时间自动重建会话两次，失败后保留手动重试。浏览器保留可切换软字幕；PotPlayer/IINA 只接收单个媒体 URL，因此选择字幕时自动改用 HLS，并在增强/模型处理后烧录当前轨道。FFmpeg 缺少 libass 时结构化拒绝。自适应码率和带宽探测仍待增强。
...
| 终版 F3 | 已完成普通 HLS 编码器回退、远程 RIFE + Real-ESRGAN rawvideo 管线、模型链逐候选编码器启动重试、浏览器软字幕、外部播放器选中轨道后增强烧录、受认证诊断刷新和网页 HLS 两次有界断线自动恢复 | 自适应码率/带宽探测、跨厂商模型编码和外部字幕真机证据待完成 |
| 终版 F4 | PC libVLC 已从播放器、安装包和工作流移除 | 仍需两个正式版本完成跨平台真机矩阵，确认 libmpv 发布稳定性 |
| 终版 F5 | F5-A 至 F5-D 已完成源码接入；F5-E 已加入 device-lost、源帧率 80% 帧预算、GPU 队列 P95、丢帧率、连续 250 ms 漂移、2 秒无帧和拖动后 8 秒恢复门禁；F5-F 已加入独立 AudioDecoder/AudioContext 时钟、播放控制和 DOM 字幕 cue 时钟；F5-G 已加入纹理维度、估算工作集和运行时 OOM 门禁；F5-H 已加入最多两次 Range 续传恢复；F5-I 已加入受认证的 `direct-enhanced` 会话诊断，不修改原文件或启动服务端转码 | 当前达到源码、静态门禁和网关契约状态；30 分钟浏览器/GPU 真机矩阵仍待完成，`supportsDirectEnhancement` 保持 `false` |
...
- 基础 HLS 已使用独立编码器探测；RIFE 与 Real-ESRGAN 处理内存 RGB24 帧，字幕不进入模型输入。
- 浏览器会话保留独立软字幕；外部播放器只接收一个 URL，选择字幕时强制 HLS，并把当前字幕滤镜追加在画质增强/模型输出之后。无字幕时保持请求的直传或转码模式。
- FFmpeg `subtitles`/libass 能力会在烧录会话前探测并缓存；缺失时返回 `MEDIA_SUBTITLE_BURN_UNAVAILABLE`，不静默降为无字幕。
- 网页 HLS 已按当前绝对时间执行最多两次会话重建；自适应码率、延迟模式和带宽探测仍待纳入远程会话契约。
- 模型链按 NVENC/AMF/QSV 选择，最后回退 libx264，并在会话诊断中标记实际编码器；每个候选都会重建完整模型管线并清理上一轮 HLS 输出。
...
4. **F5-D 控制层接入**：已实现最多 48 帧的媒体时间队列、2 秒解码前探、8 秒增强首帧门禁、关键帧拖动重启、可见画布接管和 `balanced`/`clear` 热切换。ArtPlayer 继续承载控制 UI、全屏、进度和 DOM 字幕；增强激活后播放、暂停、拖动、倍速、音量和静音由 F5-F 独立控制器执行。只有增强首帧成功后才隐藏原视频，初始化或运行失败立即恢复原画。指定 Intel Mac + AMD RX 6750 XT + Chrome 的 1080p MP4 已完成 30:40 `device-passed`，完整浏览器/GPU 矩阵仍未完成。
5. **F5-E 降级与验收**：已实现 WebGPU 初始化/device-lost、解码器、Range 上限、首帧、拖动恢复和运行错误回退原画；最近 60 个 GPU 队列样本计算 P95，最近 120 次展示统计丢帧，帧预算按源帧间隔的 80% 计算并最多取 30 FPS 的 `33.3 ms` 安全窗口。清晰档超过 75% 预算或 20% 丢帧时先降均衡；均衡超过完整预算、35% 丢帧、连续 12 帧漂移超过 `250 ms`、播放时钟前进但 2 秒无新帧时退出增强。页面诊断属性记录实际档位、预算、P95、漂移、丢帧、出帧数和原因；原视频本身失败时继续沿用 HLS 自动转码。断线恢复和服务端 `direct-enhanced` 诊断已接入，浏览器/GPU 真机矩阵仍待完成。
...
8. **F5-H Range 网络中断恢复**：每个逻辑 Range 请求最多追加两次物理重试，使用 150/300 ms 退避，覆盖建连失败、408/429/500/502/503/504、响应体异常和提前结束。续传从已交付的下一字节开始并严格核对 `Content-Range` 起点；重试仍计入全局请求次数和累计接收字节上限。401/403/416、协议错误和用户取消立即失败，最终由现有降级链恢复原视频。页面诊断记录物理重试、成功恢复和网络失败次数。
9. **F5-I 客户端到服务端诊断**：远程页面通过设备 Bearer 凭据向所属直传会话上报能力结果、实际档位、AudioContext 时钟、帧/GPU/漂移/Range 指标和降级原因；服务端限制请求频率、校验字段范围并以单调 `sequence` 拒绝迟到覆盖。只有 `active` 且能力、有效档位和 AudioContext 主时钟同时成立时，服务端 `playbackPath` 才写为 `direct-enhanced`，`idle`、探测中和降级状态保持 `direct`；HLS 会话拒绝该上报。
