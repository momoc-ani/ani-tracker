...
6. **远程增强输出**：基础 HLS 已按 NVENC/AMF/QSV/libx264 探测并报告降级；RIFE 与 Real-ESRGAN 已进入独立 rawvideo 管线，音轨和软字幕不经过模型，实际模型后端与运行时降级通过 2 秒诊断轮询展示。模型链已支持逐候选编码器启动重试；网页 HLS 致命错误会从当前绝对时间自动重建会话两次，失败后保留手动重试。终端字幕能力协商、烧录字幕、自适应码率和带宽探测仍待增强。
...
- 模型 rawvideo 链的跨厂商真机证据、终端字幕能力协商与烧录字幕、自适应码率和带宽探测。
...
| 终版 F3 | 已完成普通 HLS 编码器回退、远程 RIFE + Real-ESRGAN rawvideo 管线、模型链逐候选编码器启动重试、独立音轨/软字幕、受认证诊断刷新和网页 HLS 两次有界断线自动恢复 | 终端字幕协商与烧录字幕、自适应码率/带宽探测、跨厂商模型编码真机证据待完成 |
